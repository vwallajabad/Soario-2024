...
async function predict(type) {
  ...
  imageElement.onload = async () => {
    ...
    let alzheimerconfidence = (
      alzheimerpredictionProbabilities[alzheimerpredictedClass] * 100
    ).toFixed(2);
    let tumorconfidence = (
      tumorpredictionProbabilities[tumorpredictedClass] * 100
    ).toFixed(2);

    const getConfidenceColor = (confidence) => {
      return confidence < 33
        ? "#f55142"
        : confidence < 66
        ? "#ecff3d"
        : "#42f587";
    ...
    if (type == "alzheimer") {
      ...
      <div class="progress-label">Alzheimer Prediction: ${
        alzheimerLabel[alzheimerpredictedClass]
      }</div>
      <div class="progress-bar" style="background-color: ${getConfidenceColor(
        alzheimerconfidence
      )}; width: ${alzheimerconfidence}%;">
          <span style="color: black;">${alzheimerconfidence}%</span>
      </div>
  `;
      return 0;
    }
    document.getElementById("result").innerHTML = `
            <div class="progress-label">Tumor Prediction: ${
              tumorLabel[tumorpredictedClass]
            }</div>
            <div class="progress-bar" style="background-color: ${getConfidenceColor(
              tumorconfidence
            )}; width: ${tumorconfidence}%;">
                <span style="color: black;">${tumorconfidence}%</span>
            </div>
        `;
  };
}
